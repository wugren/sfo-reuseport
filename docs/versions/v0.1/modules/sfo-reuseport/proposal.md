...
- `TcpServer`、`UdpServer` 和 `QuicServer` 必须支持通过 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 配置常规 handler 已交付工作的最大并发数；该并发数在每个 worker 线程内独立计算，默认不做限制，显式配置为 0 也表示不限制；每个 worker 各自维护许可，不做全局共享池；达到上限时必须在 accept/recv 前等待已有 handler 完成并释放新许可，不得主动丢弃连接或数据包；server 关闭时正在等待许可的 accept/recv 必须解除等待并退出。
...
- 未决歧义：
  ...
  - `TcpServer`、`UdpServer` 和 `QuicServer` 的并发数限制已确认使用 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 配置；默认不限制，显式 0 表示不限制；每个 worker 各自维护许可，不做全局共享池；达到上限时在 accept/recv 前等待新许可；server 关闭时正在等待许可的 accept/recv 解除等待并退出。设计阶段仍需定义具体类型、builder/API 形态、错误路径和 runtime feature 下的等待实现。
...
- 禁止方案：
  ...
  - 并发数限制只能约束常规 handler 型 `serve` 已交付的用户 future 数量，不得改变 worker 数量、不暴露 worker id，不得为 socket-only `serve_socket` 增加 crate 接管的应用收包循环限流。
...
- 用户可以通过 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 为 `TcpServer`、`UdpServer` 和 `QuicServer` 的常规 handler 型服务设置每个 worker 线程独立计算的最大并发数，避免单个 worker 无限启动用户 handler future；默认不设置或显式配置为 0 时不做限制并维持现有行为。
...
| P-server-concurrency-limit | CHG-server-concurrency-limit | `TcpServer`、`UdpServer` 和 `QuicServer` 支持通过 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 限制常规 handler 型服务中已交付用户 handler future 的最大并发数；该上限按每个 worker 线程独立计算，默认不限制，显式 0 也表示不限制。 | `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 配置表面、TCP accept/handler 投递边界、UDP packet handler 投递边界、QUIC-aware UDP packet handler 投递边界、每 worker 线程独立许可等待与释放、关闭并发和错误语义；每个 worker 各自维护许可，不做全局共享池；达到上限时在 accept/recv 前等待新许可；server 关闭时正在等待许可的 accept/recv 解除等待并退出。 | 测试或可运行验证展示设置并发数后每个 worker 线程内 TCP 同时运行的连接 handler 不超过上限，UDP 和 QUIC-aware UDP 同时运行的 packet handler 不超过上限；达到上限时在 accept/recv 前等待该 worker 释放新许可，不主动丢弃连接或数据包；handler 完成后释放该 worker 的许可并允许后续工作进入；默认配置和显式 0 均不限制并发；关闭 server 后等待许可的任务退出且不泄漏许可或阻塞 runtime；API 编译覆盖证明配置属于 typed service config 而不是 worker 数量。 | 不提供请求速率限制、令牌桶、超时、重试、优先级队列、跨 server 全局配额、按客户端配额或自适应负载调度；不提供跨 worker 共享并发池；不为 `serve_socket` 增加 crate 级应用读取并发控制；不改变 worker 数量配置所有权。 |
...
- 用户可见或系统可见结果：
  ...
  - `TcpServer`、`UdpServer` 和 `QuicServer` 的常规 handler 型服务支持通过 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 配置每个 worker 线程独立计算的最大并发数；达到上限时该 worker 在 accept/recv 前等待已有 handler 完成并释放新许可，不主动丢弃连接或数据包；默认配置和显式 0 均保持不限并发；server 关闭时等待许可的 accept/recv 解除等待并退出。
...
| runtime/integration | yes | 新增 `TcpServer`、`UdpServer` 和 `QuicServer` 常规 handler 并发数限制会改变 listener loop、handler 投递、完成释放和关闭并发边界。 | design 需定义 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 的具体类型和 builder/API 形态、默认不限制、显式 0 不限制、每个 worker 线程独立计数、accept/recv 前等待新许可、许可释放和 close/drop 解除等待边界；post-implementation testing 需覆盖上限命中、释放后继续处理、默认和 0 不限、关闭不泄漏和三种 server 类型。 | none | design 阶段先补齐接口与运行时边界；实现后 testing 阶段补齐验证。 | 若许可等待与 handler panic/错误路径耦合，可能导致连接饥饿、数据包无界排队或永久阻塞。 |
...
| FU-057 | design | 定义 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker` 的具体类型和 builder/API 形态；同步 `TcpServer`、`UdpServer` 和 `QuicServer` 常规 handler 并发数限制的默认不限、显式 0 不限、每个 worker 线程独立计数、accept/recv 前等待新许可、许可释放、错误路径和 close/drop 解除等待边界，并说明 `serve_socket` 不纳入 crate 级 handler 并发控制。 | P-server-concurrency-limit/P-tcp/P-udp/P-quic-routed-udp/P-dynamic-listeners | yes |
| FU-058 | testing | 为 `CHG-server-concurrency-limit` 增加测试计划和 API 编译契约，覆盖 `TcpServiceConfig::max_concurrency_per_worker` 或 `UdpServiceConfig::max_concurrency_per_worker`、TCP/UDP/QUIC-aware UDP 的每 worker 线程并发上限、上限命中后在 accept/recv 前等待新许可且不主动丢弃连接或数据包、handler 完成释放本 worker 许可、默认和显式 0 不限并发、关闭时等待许可的 accept/recv 退出、不改变 worker 数量配置所有权，以及 socket-only serve 不被该限制接管。 | P-server-concurrency-limit | yes |
| FU-059 | implementation | 仅在 proposal/design 重新批准且 `CHG-server-concurrency-limit` 实现准入通过后，在 `TcpServer`、`UdpServer` 和 `QuicServer` 常规 handler 投递路径中实施并发数限制；相关测试代码和测试元数据在后续 testing 阶段同步。 | P-server-concurrency-limit | yes |
