...
| UDP/QUIC socket-only serve | `UdpServer::serve_socket` 和 `QuicServer::serve_socket` 通过回调交付统一监听 `UdpSocket`，返回 server 生命周期对象，不调用数据包 handler。 | unit/integration | API 编译测试确认两个 socket-only serve 入口必须传入 `&ServerRuntime`、`ServiceConfig` 和 socket 回调，返回 `UdpServer`/`QuicServer`；integration 测试确认应用可在回调获得的 socket 上自行 `recv_from`；fallback 验证需确认每个 worker socket 视图只接收对应 worker 应收数据。 | automated | `tests/unit/api_signatures.rs`、`tests/integration/udp_serve.rs` | ready | |
...
| UDP/QUIC socket-only serve 不应读取数据或调用 handler。 | API 编译测试和 socket-only loopback integration test。 | 测试直接通过回调收到的 `UdpSocket` 自行 `recv_from`，证明读取由应用负责；`QuicServer::serve_socket` 接收可路由 QUIC payload，证明应用可自行处理协议数据；fallback 后端允许为隔离 worker socket 视图执行内部收包与分流。 | |
...
| callback signatures and server entrypoints | TCP/UDP/QUIC handler 不包含 worker id；`TcpServer`、`UdpServer`、`QuicServer` 只通过显式 `&ServerRuntime` 的同步 `serve` 调用，返回各自 server 对象，生产代码不使用 `pending` 挂起。 | `tests/unit/api_signatures.rs` |
...
| UDP unified socket API | handler 接收统一 `UdpSocket`，`serve_socket` 回调交付统一 `UdpSocket`，crate 根不导出 `BalancedUdpSocket`。 | `tests/unit/api_signatures.rs` |
...
| UDP/QUIC socket-only serve | `UdpServer::serve_socket` 和 `QuicServer::serve_socket` 回调交付统一 socket，应用自行 `recv_from`。 | `tests/integration/udp_serve.rs` |
...
- `BalancedUdpSocket` 不得重新出现在公开 API；UDP/QUIC handler、`listener_socket` 和 `serve_socket` 应继续接收或回调交付统一 `UdpSocket`。
...
| FU-TEST-015 | acceptance | socket-only serve 实现后审计 proposal、design、代码、API 测试和 integration 测试是否一致，特别检查该入口通过回调交付 socket、fallback 每 worker socket 视图不暴露全量共享收包口且 Windows/fallback 机制不进入公开 API。 | UDP/QUIC socket-only serve | yes |
