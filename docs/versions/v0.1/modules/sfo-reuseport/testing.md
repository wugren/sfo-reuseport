...
| QUIC routed UDP | `QuicServer` 对 Initial/0-RTT 使用 DCID hash fallback，对已携带本端 CID 的 packet 按 QUIC DCID 开头的固定 2 字节 worker index 前缀稳定分配 UDP packet，不提供 QUIC 协议栈 API。 | unit/integration | long/short header worker index 前缀被解析；worker index 使用 16-bit 网络字节序编码；server Initial/0-RTT 至少 8 字节 DCID 按 hash fallback 稳定选择 worker；fallback worker 与后续前缀 CID 路由一致；packet 投递到对应 worker；DCID 短于边界、非法或缺失路由键不调用 handler；集成测试通过 `SFO_REUSEPORT_DISABLE_QUIC_BPF` 固定验证用户态 fallback，BPF 指令和 probe shape 保留在 unit 覆盖。 | automated | `tests/unit/quic_routed_udp.rs`、`tests/integration/quic_routed_udp.rs` | ready | |
...
| QUIC Linux reuse-port BPF selector | Linux 上 best-effort 优先附加 reuse-port eBPF selector，失败或 selector 与用户态 Initial/0-RTT fallback 算法不一致时退回 CBPF，再失败时保持用户态 QuicServer fallback。 | unit/dv/integration | eBPF 程序指令生成、`BPF_PROG_TYPE_SK_REUSEPORT` load 属性、`SO_ATTACH_REUSEPORT_EBPF` attach 路径、CBPF fallback、Initial/0-RTT hash fallback 一致性探测和 worker modulo 覆盖；当前平台编译覆盖平台 cfg；loopback integration 在 selector 可用或 fallback 时均保持稳定投递。 | automated/manual | `src/platform/mod.rs` 内部 unit、`tests/dv/platform_cfg.rs`、`tests/integration/quic_routed_udp.rs` | ready | 非 Linux 只能验证 fallback/cfg；内核拒绝 eBPF/CBPF 加载或附加时以 fallback 行为作为自动验证证据。 |
...
| CHG-quic-routed-udp | `design.md` | VAL-explicit-runtime-serve-api | unit | explicit-runtime-serve-api | no | 包含 QUIC Initial/0-RTT DCID hash fallback、route key parsing、worker 稳定投递、Linux reuse-port eBPF selector best-effort、CBPF fallback 和用户态 fallback 证据；DV step `quic-reuseport-bpf` 提供平台 selector 编译覆盖。 |
...
| worker 数量必须属于 `ServerRuntime`。 | unit/API 测试断言 `ServerRuntimeConfig` 拥有 worker 配置，`ServiceConfig` 不支持 worker 设置；integration test 通过 `serve` 在同一 `ServerRuntime` 中注册 TCP/UDP listener。 | 公开 API 表面和运行期混合 listener 行为共同证明 worker 配置在 runtime 层共享。 | |
...
| QUIC 路由字段来自不可信网络输入。 | unit 测试覆盖短包、空 DCID、Initial/0-RTT DCID hash fallback、fallback DCID 短于 8 字节、非 fallback 固定 2 字节 worker index 前缀、长度越界、eBPF selector fallback 选择；integration 测试覆盖合法 long header worker index 前缀和握手 fallback worker 稳定性。 | 长度检查和丢弃语义是防 panic 和防错误 handler 调用的直接边界；2 字节前缀缺少第二字节的负例强制外部遵守 layout；eBPF/CBPF 只作为内核预分配优化，自动测试允许当前内核拒绝或一致性探测失败后走 fallback。 | |
...
| QUIC route key parsing | Initial/0-RTT DCID hash fallback、long header 固定 2 字节 worker index 前缀、short header 固定 2 字节 worker index 前缀、非法长度、fallback 短 DCID、2 字节前缀截断和空 DCID。 | `tests/unit/quic_routed_udp.rs` |
...
| quinn feature check | 默认 runtime、async-std runtime 和 tokio-uring runtime 下 `quinn` feature 编译，覆盖非 tokio runtime 的 unsupported 编译边界。 | `harness/scripts/test-run.py` dv |
...
| QUIC routed UDP worker stability | `QuicServer` 将 Initial/0-RTT fallback packet 和带 DCID worker index 前缀的 packet 投递到对应 worker。 | `tests/integration/quic_routed_udp.rs` |
...
- `QuicServer` 不得新增 TLS、connection 或 stream API；非法 QUIC 路由 packet、短于 8 字节的 Initial/0-RTT fallback DCID，或不满足 worker index 前缀 layout 的非 fallback packet 不得调用 handler。
...
| FU-TEST-009 | testing | 增加 `QuicServer` 测试，覆盖 QUIC Initial/0-RTT DCID hash fallback、route key parsing、稳定 worker 投递、Linux reuse-port BPF selector best-effort 路径、fallback 和非法 packet 丢弃。 | QUIC routed UDP | yes |
