...
approved_at: 2026-06-10T17:39:21+08:00
---
...
| worker-local mutable state | `TcpServer`、`UdpServer` 和 `QuicServer` 常规 handler 的用户自定义每 worker state。 | unit/integration/source-review | `serve_with_state` 入口接受用户自定义 `S: Clone + 'static` 且不要求 `Send`/`Sync`；`state_factory` 在每个 server 的每个 worker 内创建 state；handler 接收同一 worker state clone，可用 `Rc<RefCell<_>>` 写入；普通 handler 签名仍不包含 worker id；TCP/UDP/QUIC loopback 证明同 worker packet/connection 复用可写 state；TCP close/drop 后 crate 持有的 worker state 可释放；源码审查确认 fallback 路径通过 worker-local work queue 分发，不在热路径 thread-local map 反查 state。 | automated | `tests/unit/api_signatures.rs`、`tests/integration/worker_local_state.rs`、`tests/integration/udp_serve.rs`、`tests/integration/quic_routed_udp.rs`、`src/core/tcp.rs`、`src/core/udp.rs`、`src/core/quic.rs` | ready | |
...
| `serve_with_state` | TCP/UDP/QUIC 常规 handler 的每 worker 用户自定义可写 state。 | `TcpServer::serve_with_state`、`UdpServer::serve_with_state` 和 `QuicServer::serve_with_state` 可用 `Rc<RefCell<_>>` 作为 state；同一 worker 多个连接或 packet 观察到递增 state；基础 `serve` 保持兼容并委托到 unit state；server close/drop 后 crate 内部 worker state 引用释放。 | 常规 handler 不暴露 worker id；state 不跨 worker 共享；用户若跨 `.await` 持有可变借用需自选同步结构；crate 不提供跨 worker state 访问 API。 | unit/integration/source-review | `tests/unit/api_signatures.rs`、`tests/integration/worker_local_state.rs`、`tests/integration/udp_serve.rs`、`tests/integration/quic_routed_udp.rs`、`src/core/tcp.rs`、`src/core/udp.rs`、`src/core/quic.rs` | ready | |
...
| CHG-worker-local-state | `design.md` | VAL-worker-local-state | unit/integration | worker-local-state-api / worker-local-state-serve | no | API 编译覆盖 `Rc<RefCell<_>>` state 类型、三类 `serve_with_state` 入口和常规 handler 无 worker id；integration 覆盖 TCP close 后释放、TCP/UDP/QUIC 同 worker state 复用和可写状态；fallback work queue/state 持有路径以源码审查覆盖当前平台不可强制的 TCP/UDP fallback。 |
...
| worker-local state API | `TcpServer::serve_with_state`、`UdpServer::serve_with_state`、`QuicServer::serve_with_state` 接受用户自定义 `Rc<RefCell<_>>` 可写 state，state 类型不要求 `Send`/`Sync`。 | `tests/unit/api_signatures.rs` |
...
| TCP worker-local state | `serve_with_state` 每 worker 初始化一次、同 worker 复用可写 `Rc<RefCell<_>>` state，server close/drop 后释放 crate 持有的 state。 | `tests/integration/worker_local_state.rs` |
...
| UDP worker-local state | `UdpServer::serve_with_state` 同 worker packet 复用可写 state 并用统一 `UdpSocket` 响应。 | `tests/integration/udp_serve.rs` |
...
| QUIC worker-local state | `QuicServer::serve_with_state` 在 QUIC routed UDP handler 中复用同一 worker 的可写 state。 | `tests/integration/quic_routed_udp.rs` |
...
- `TcpServer::serve_with_state`、`UdpServer::serve_with_state`、`QuicServer::serve_with_state` 必须保持用户自定义 `S: Clone + 'static` state，不要求 `S: Send`/`Sync`；state 只能在目标 worker 创建和复用，常规 handler 不得通过 worker id 或 thread-local map 反查 state。
...
| FU-TEST-023 | acceptance | worker-local state 实现后审计 proposal、design、代码、API 测试和 integration 测试是否一致，特别检查 state 类型由用户定义、可写、每 worker 初始化一次、同 worker 复用、常规 handler 无 worker id、关闭后释放以及 fallback 不通过 thread-local map 反查。 | worker-local mutable state | yes |
