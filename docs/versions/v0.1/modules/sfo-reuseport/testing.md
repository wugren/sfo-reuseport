...
approved_at: 2026-06-03T11:37:12+08:00
---
...
| tokio-uring runtime feature | `runtime-tokio-uring` 互斥 feature、Linux cfg、公开网络 socket 类型和 handler API。 | unit/dv | tokio-uring feature 可在 Linux 下编译；与其他 runtime feature 互斥；公开 `TcpStream` 使用 tokio net 类型且保持 `Send`，统一 `UdpSocket` 可导入并使用 tokio UDP I/O 表面；tokio-uring all-targets 编译覆盖 handler 中不依赖 tokio-uring net API 的类型边界。 | automated | `tests/unit/runtime_features.rs`、`cargo check --no-default-features --features runtime-tokio-uring --all-targets` via `test-run.py` dv | ready | 非 Linux 由 compile_error 边界覆盖，需要目标平台编译验证。 |
...
| server runtime random task submission | `ServerRuntime` 公开随机 worker task 投递接口。 | unit | `ServerRuntime::spawn` 直接接受调用方 future 并投递到已有 worker runtime，返回 task handle；公开签名不使用 `FnOnce() -> Fut` task factory；单 worker 配置下任务在线程外的 worker runtime 执行；内部 inactive/shutdown flag 清除后投递返回明确 runtime error；`ServerRuntime` 不公开指定 worker 投递方法。 | automated | `src/core/server_runtime.rs` 内部 unit、`tests/unit/server_runtime.rs` | ready | |
...
| tokio-uring runtime API 与 tokio 网络接口集成边界不一致。 | Linux tokio-uring feature all-targets 编译、公开类型导入测试、tokio-uring feature 下 examples 编译和 runtime adapter 编译。 | 验证重点是 `runtime-tokio-uring` 不暴露 tokio-uring 原生 net 类型，公开 `TcpStream` 保持 tokio net 类型和 `Send`，统一 `UdpSocket` 使用 tokio UDP I/O 表面。 | 非 Linux 运行行为不承诺，使用 compile_error 边界。 |
...
| tokio-uring runtime feature | `runtime-tokio-uring` 下公开 `TcpStream` 使用 tokio net 类型且保持 `Send`，统一 `UdpSocket` 可导入并保持 `Send`。 | `tests/unit/runtime_features.rs` |
...
| server runtime random task submission | `ServerRuntime::spawn` 直接接受 future，把任务投递到已有 worker runtime，且不公开指定 worker 投递接口。 | `tests/unit/server_runtime.rs` |
...
| quinn feature check | 默认 runtime、async-std runtime 和 tokio-uring runtime 下 `quinn` feature 编译，覆盖 tokio-uring 复用 tokio network helper 和 async-std unsupported 编译边界。 | `harness/scripts/test-run.py` dv |
...
| FU-TEST-013 | acceptance | tokio-uring runtime feature 实现后审计 feature 互斥、Linux cfg、公开网络 socket 类型保持 tokio net、handler 不依赖 tokio-uring net API 和测试入口证据。 | tokio-uring runtime feature | yes |
