#!/usr/bin/env python3
"""Run the udp_server example and verify UDP echo behavior."""
...
def wait_for_echo(port: int, process: subprocess.Popen[bytes]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as client:
        ...
        while time.monotonic() < deadline:
            ...
            if peer[0] == "127.0.0.1" and peer[1] == port and data == b"ping":
                return
...
def main() -> int:
    ...
    print(f"udp_server example smoke test passed for {args.runtime}")
    return 0
