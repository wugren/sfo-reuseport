...
{
    Ok(TaskHandle {
        inner: TaskHandleInner::Local(tokio_uring::spawn(future)),
    })
}
...
pub struct CurrentThreadExecutor {
    task_sender: tokio::sync::mpsc::UnboundedSender<TaskCommand>,
    task_receiver: Option<tokio::sync::mpsc::UnboundedReceiver<TaskCommand>>,
...
impl CurrentThreadExecutor {
    ...
    pub(crate) fn spawn_task(&self, task: ExecutorTask) -> io::Result<TaskHandle> {
        self.handle().spawn_task(task)
    }

    pub(crate) fn park_until_shutdown(mut self, shutdown: ShutdownReceiver) {
        let mut task_receiver = self
            .task_receiver
            .take()
            .expect("worker task receiver should exist");
    ...
    pub(crate) fn spawn_task(&self, task: ExecutorTask) -> io::Result<TaskHandle> {
        if thread::current().id() == self.owner_thread {
            return Ok(TaskHandle {
                inner: TaskHandleInner::Local(tokio_uring::spawn(task)),
            });
        }
